...
like_button ="<button class='btn btn-warning' id="+firebase_message_id+" value="+like99+" onclick='updateLike(this.id)'>";
span_with_tag = "<span class='glyphicon glyphicon-thumbs-up'>Like: "+ like99 +"</span></button><hr>";
row = div09 + msg66 + like_button + span_with_tag;
document.getElementById("output").innerHTML = row;
} }); }); } 
getData();

function updateLike(like_number){
    console.log("User has clicked the like button, the current numebr of likes are" + like99);
    button_id = like_number;
    like_no = document.getElementById(button_id).value;
    updated_likes =  Number(like_no) + 1;
    console.log(updated_likes);

    firebase.database().ref(room_19).child(like_number).update({
        like: updated_likes
    });
}